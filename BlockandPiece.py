...
class Piece:
    ...
    def set_block_state(self, block_index: int, state: Status, data: bytes = None, last_requested = None, peer: Peer = None):
            ...
            with self._block_requested_by_lock.write_access:
                ...
                if state == Status.EMPTY:
                    for prev_peer in requested_by:
                        prev_peer.cancel_block(self._piece_index, block_index)
                    requested_by.clear()
                    if peer:
