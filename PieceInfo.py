...
class PieceInfo:
    def __init__(self, torrent):
        ...
        self._torrent_lock = RWLock()
        self._number_of_pieces = ceil(torrent.total_length / torrent.piece_length)
        self._number_of_pieces_lock = RWLock()
        self._pieces: list[Piece] = []
        self._pieces_lock = RWLock()
        self._pieces_SHA1 = []
        self._pieces_SHA1_lock = RWLock()
        self.getSHA1()
        self.generate_piece()
        self._totalBlocks = self.getTotalBlocks()
        self._totalBlocks_lock = RWLock()
        self._files = self._load_files()
        self._files_lock = RWLock()
        self._last_update_time = time.time()
        self._last_update_time_lock = RWLock()
        self._last_blocks_done = 0
        self._last_blocks_done_lock = RWLock()
        self._last_bytes_done = 0
        self._last_bytes_done_lock = RWLock()
        self._download_speed = 0
        self._download_speed_lock = RWLock()
        self._speed_history = []
        self._speed_history_lock = RWLock()

    @property
    def torrent(self):
        with self._torrent_lock.read_access:
            return self._torrent
    @torrent.setter
    def torrent(self, value):
        with self._torrent_lock.write_access:
            self._torrent = value
    ...
    def write_into_file(self):
        ...
        if self.torrent.multipleFiles:
            while master_i < n:
                ...
                if not piece or not piece.is_complete() or not piece.verify_piece():
                    master_i += 1
                    continue
            ...
            try:
                ...
                while master_i < n:
                    piece = self.get_piece_safe(master_i)
                    if not piece or not piece.is_complete() or not piece.verify_piece():
                        master_i += 1
                        continue
    ...
    def piecesDownloaded(self):
        ...
        for i in range(len(self.pieces)):
            ...
            if piece and piece.is_complete(): 
                done += 1
        return done
    ...
    def download_blocks_safe(self):
        summary = self.get_pieces_summary_safe()
        return summary['completed_blocks'], summary['requested_blocks']

    def all_piece_complete_safe(self):
        summary = self.get_pieces_summary_safe()
        return summary['completed_pieces'] == summary['total_pieces']
    ...
    def print_progress_bar_safe(self, iteration, total, suffix='Complete', decimals=1, print_matrix=False, peers=None):
        summary = self.get_pieces_summary_safe()
        pieces_copy = summary.copy()
        matrix_copy = None
        return self._print_progress_bar_internal(
            iteration, total, suffix, decimals, print_matrix, peers, pieces_copy, matrix_copy
        )
    ...
    def get_pieces_summary_safe(self):
        pieces = self.pieces
    ...
    def merge_blocks_safe(self, index):
        return self.merge_blocks(index)

    def get_total_blocks_safe(self):
        return self.getTotalBlocks()
            ...
            while master_i < n:
                ...
                try:
                    ...
                    try:
                        ...
                        f = open(path_to_file, 'wb')
                    piece_data = self.merge_blocks_safe(piece_index)
                    if piece_data is not None:
                        data_to_be_written = piece_data[piece_offset : piece_offset + length]
                        f.seek(file_offset)
                        f.write(data_to_be_written)
            ...
            try:
                ...
                while master_i < n:
                    ...
                    if not piece_status or not piece_status['is_complete']:
                        master_i += 1
                        continue
                    piece_data = self.merge_blocks_safe(master_i)
                    if piece_data is not None:
                        f.write(piece_data)
                    master_i += 1
            except PermissionError as e:
